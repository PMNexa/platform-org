 ...
 *
 * Just the `Organization` type - `apps/main` owns every actual URL for
 * this resource itself now (`createOrgsRoutes(basePath)`, this
 * package's own `"./routes"` Node-only subpath, called with whatever
 * mount prefix `apps/main`'s own `routes.ts` decides; the sidebar nav
